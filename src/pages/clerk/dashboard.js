...
const ClerkDashboard = () => {
  ...
  useEffect(() => {
    ...
    // Set interval to refresh issues every 10 seconds for real-time updates
  ...
  const IssueCard = ({ issue }) => (
    <div 
      ...
      className="bg-white rounded-lg p-6 flex justify-between items-center cursor-pointer hover:shadow-md border border-gray-100"
    >
      <div>
        <h3 className="text-2xl font-semibold text-gray-900 mb-2">{issue.title}</h3>
        <p className="text-gray-700">Filed on: {new Date(issue.createdAt).toLocaleDateString()}</p>
      </div>
  ...
  const StatusSection = ({ status, bgColor }) => (
    <div className="mb-8">
      <div className={`rounded-md px-6 py-3 mb-4`} style={{ backgroundColor: bgColor }}>
  ...
  // Revert to original pastel background colors for statuses
  const statusColors = {
    "Pending Approval": "#d0e7f9", // light blue
    "Under Review": "#fff9d6",     // light yellow
    "Resolved": "#e6f4ea",         // light green
    "Rejected": "#f9d6d6"          // light red
  };
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-[1400px] mx-auto px-6 py-8">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-[#FF6B00] text-4xl font-bold">Clerk Dashboard</h1>
        ...
        <div className="flex gap-8">
          <div className="flex-1">
            <StatusSection status="Pending Approval" bgColor="#E6F0F9" />
            <StatusSection status="Under Review" bgColor="#FFF8E6" />
            <StatusSection status="Resolved" bgColor="#E7F6EC" />
            <StatusSection status="Rejected" bgColor="#FFE6E6" />
          </div>

          <div className="w-[400px]">
            <div className="bg-black text-white p-8 rounded-xl">
              <h2 className="text-[#FF6B00] text-3xl font-bold mb-12">Statistics</h2>
              <div className="grid grid-cols-2 gap-x-12 gap-y-12">
                <div>
                  <p className="text-xl mb-2">Total Issues</p>
                  <p className="text-4xl">{stats.total}</p>
                </div>
                <div>
                  <p className="text-xl mb-2">Pending Approval</p>
                  <p className="text-4xl">{stats.pendingApproval}</p>
                </div>
                <div>
                  <p className="text-xl mb-2">Under Review</p>
                  <p className="text-4xl">{stats.underReview}</p>
                </div>
                <div>
                  <p className="text-xl mb-2">Resolved</p>
                  <p className="text-4xl">{stats.resolved}</p>
                </div>
                <div>
                  <p className="text-xl mb-2">Rejected</p>
